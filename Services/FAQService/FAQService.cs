...
namespace AngularAdminPannel.Services.FAQService
{
    public class FAQService:IFAQService
    {
        ...
        public async Task<PagedResult<FAQListItemDto>> GetFAQsAsync(FAQListFilterDto filter)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filter.Filter) && !string.IsNullOrWhiteSpace(filter.Search))
            {
                ...
                {
                    query = query.Where(u => u.Question!.Contains(searchUpper));
                    hasCustomOrder = true;
                }
                else 
                if (filterType == "DISPLAYORDER")
                {
                    query = query.Where(u => u.DisplayOrder.ToString().Contains(searchUpper));
                    hasCustomOrder = true;

                }

                else if (filterType == "ORDERQUESTION")
                {
                    if (searchUpper == "ASC")
                    {
                        query = query.OrderBy(u => u.Question);
                        hasCustomOrder = true;
                    }
                    else
                    {
                        query = query.OrderByDescending(u => u.Question);
                        hasCustomOrder = true;
            ...
            var faqList = await query
                ...
                .ToListAsync();
            return new PagedResult<FAQListItemDto>
            {
                Items = faqList,
        ...
        public async Task<(IdentityResult Result, Guid? RoleId)> CreateAsync(FAQCreateDto model)
        {
            if (await _context.FAQs.AnyAsync(r => r.Question == model.Question))
            {
                return (IdentityResult.Failed(new IdentityError { Description = "FAQ with this Question already exists." }), null);
            }
        ...
        public async Task<FAQEditDto?> GetForEditAsync(Guid id)
        {
        ...
        public async Task<IdentityResult> UpdateAsync(FAQEditDto model)
        {
            if (await _context.FAQs.AnyAsync(r => r.Question == model.Question && r.Id != model.Id))
            {
                return (IdentityResult.Failed(new IdentityError { Description = "FAQ with this Question already exists." }));
            }
        ...
        public async Task<IdentityResult> ToggleActive(Guid id)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Code = "SaveFailed",
                    Description = "Could not update FAQ."
                });
            }
        }
